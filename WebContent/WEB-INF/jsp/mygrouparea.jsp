...
<div class="bg">
	<form action="${pageContext.request.contextPath}/newupload.action?groupname=${group_name}" method="post" enctype="multipart/form-data">
	    <div><input type="file" onchange="checkfile()" id="fileupload" name="file" onpropertychange="getFileSize(this.value)"/></div>
	    <div><input type="hidden" name="groupname" value="${group_name}" /><div>	
	    <div><input type="submit" onclick="return checkfile()" value="上传文件" style="background-color: #3B89F5;border-color: #3B89F5;float:left;"/></div>
	    <div style="color:white; float:left">${message }</div>
  	</form>
  	 <br/>
  	 <br/>
  	 
   	 <div style="font-size: 30px ; color: white; font-style: italic;"><font>Your Files In Group</font></div>
   	 <div style="font-size: 20px; font-style: oblique; float:inherit;
   	 		color:#3B89F5 ">${group_name}</div>
     <br/>
     
     <a href="${pageContext.request.contextPath}/searchgroupuser.action?username=${user_name}&groupname=${group_name}" 
     style="background-color: #3B89F5;border-color: #3B89F5;">查看小组成员</a>
  

    
     <script type="text/javascript">
     jQuery(function(){
    		$(".flip").click(function(){$(".waibianju").slideToggle("slow");});
    		});
     ...
     <tr>
    	...
    	<c:forEach var="c" items="${requestScope.pagebean_groupfile.list}" varStatus="stat">
    		<tr class="${stat.count%2==0?'even':'old'}">
	    		...
	    		<td>
                  <a href="javascript:void(0)" onclick="godelete(${pagebean_groupfile.currentpage},${c.id})" style="color:black;">->删除文件</a>
	    		</td>
    		<tr>
    	</c:forEach>
